...
describe('Credit Vaults API', () => {
  ...
  describe('GET /api/credit-vaults', () => {
    ...
    it('should filter vaults by chain ID', async () => {
      ...
      expect(data.every((vault: unknown) => (vault as { chainId: ChainId }).chainId === ChainId.ETHEREUM)).toBe(true);
    });
    ...
    it('should filter vaults by status', async () => {
      ...
      expect(data.every((vault: unknown) => (vault as { status: VaultStatus }).status === VaultStatus.ACTIVE)).toBe(true);
    });
    ...
    it('should filter vaults by agent ID', async () => {
      ...
      expect(data.every((vault: unknown) => (vault as { agentId: string }).agentId === 'agent_1')).toBe(true);
    });
